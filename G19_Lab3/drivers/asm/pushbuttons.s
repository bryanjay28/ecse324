...
read_PB_edgecap_ASM:				//this subroutine on access the pushbutton edgecapture register
			LDR R1, =PB_EDGECAP
			LDR R0, [R1]		// load the data into R0 
			AND R0, R0, #0xF	// return only the edge bits which is the last 4
			BX LR 				// return data and branch back

PB_edgecap_is_pressed_ASM:			//this subroutine on access the pushbutton edgecapture register
...
PB_clear_edgecp_ASM:				//this subroutine on access the pushbutton edgecapture register
			LDR R1, =PB_EDGECAP 	//load the address
			MOV R0, #0xF		// move all 1's to address of edgecapture in order to reset it 
			STR R0, [R1]
			BX LR
			
enable_PB_INT_ASM:				//this subroutine on access the pushbutton interrupt mask register
			LDR R1, =PB_MASK	
			AND R2, R0, #0xF	// we only want the last 4 numbers since there are b
			STR R2, [R1]		// store it back into the PB_MASK to enable
			BX LR			

disable_PB_INT_ASM:				//this subroutine on access the pushbutton interrupt mask register
			LDR R1, =PB_MASK
